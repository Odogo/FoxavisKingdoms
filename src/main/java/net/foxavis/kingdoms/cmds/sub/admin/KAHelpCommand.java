...
public class KAHelpCommand extends KingdomSubcommand {

	public KAHelpCommand() {
		super("admin help", "Shows the admin commands", "admin help [p:<page> l:<per page>]", "foxavis.kingdoms.admin");
	}
	...
	@Override protected boolean execute(FoxavisKingdoms plugin, CommandSender sender, String[] args) {
		try {
			...
			int limit = Arrays.stream(args) // Get the limit from the arguments, or default to 10
					...
					.map(Integer::parseInt) // Parse the argument as an integer
					.findAny() // Get the first value in the list
					.orElse(11); // If there are no values in the list, default to 10
			...
			List<KingdomSubcommand> subcommands = KingdomCommand.getInstance().getSubcommands()
					...
					.sorted(Comparator.comparing(KingdomSubcommand::getName))
					.toList();
...
//				sender.sendMessage(ChatColor.GOLD + "/k " + ChatColor.YELLOW + subcommand.getUsage() + ChatColor.RESET + " - " + ChatColor.GRAY + subcommand.getDescription());
	...
	@Override protected @Nullable List<String> tabComplete(FoxavisKingdoms plugin, CommandSender sender, String[] args) {
		List<KingdomSubcommand> subcommands = KingdomCommand.getInstance().getSubcommands()
				.stream()
				.filter(subcommand -> subcommand.getPermission() != null && subcommand.getPermission().startsWith("kingdoms.admin"))
				.filter(subcommand -> !subcommand.getName().equalsIgnoreCase("admin"))
				.toList();
